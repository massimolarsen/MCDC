...
def geant4_output_path(filename):
    return str(EXAMPLE_DIR / filename)
...
obc_sv = mcdc.Cell(region=box(*obc_sv_bounds), fill=m_silicon)
...
eps_sv = mcdc.Cell(region=box(*eps_sv_bounds), fill=m_licoo2)
...
adcs_sv = mcdc.Cell(region=box(*adcs_sv_bounds), fill=m_silicon)
...
comms_sv = mcdc.Cell(region=box(*comms_sv_bounds), fill=m_silicon)
...
mcdc.settings.output_name = str(EXAMPLE_DIR / "cubesat_CE_G4")
mcdc.run()
