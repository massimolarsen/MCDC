...
def source_energy_distribution(file, region):
    tally_path = f"tallies/{region}_electron_current"
...
def plot_source_current(mcdc_path, output_path):
    ...
    with h5py.File(mcdc_path, "r") as file:
        for region, linestyle in zip(REGIONS, LINESTYLES):
            ...
            has_positive_current |= np.any(energy_totals > 0)
            ax.stairs(
                energy_totals,
                energy_edges,
                label=region,
    ...
    ax.set_xscale("log")
    if has_positive_current:
        ax.set_yscale("log")
    else:
        ax.set_ylim(0, 1)
        ax.text(0.5, 0.5, "No current-in crossings recorded",
                transform=ax.transAxes, ha="center", va="center")
    ax.set_xlabel("Electron energy [eV]")
    ax.set_ylabel("MCDC current-in [crossings/source]")
    ax.set_title("CubeSat MCDC Electron Current by Energy")
...
def plot_spatial_current(mcdc_path, output_path):
    ...
    fig.suptitle("CubeSat MCDC Electron Current by Face")
    fig.colorbar(
        image,
        ax=axes,
        shrink=0.85,
...
def main():
    ...
    if args.output is None:
        args.output = DIAG_DIR / "electron_current_by_energy.png"
    if args.spatial_output is None:
        args.spatial_output = DIAG_DIR / "electron_current_by_face.png"
