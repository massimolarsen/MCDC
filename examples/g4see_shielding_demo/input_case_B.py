...
shield_thickness = 5.0 * MM_TO_CM
shield_inner = box_region("shield_inner", (-0.2, 0.4), (-1.2, 1.2), (-1.2, 1.2))
shield_outer = box_region(
    "shield_outer",
    (-0.2 - shield_thickness, 0.4 + shield_thickness),
    (-1.2 - shield_thickness, 1.2 + shield_thickness),
    (-1.2 - shield_thickness, 1.2 + shield_thickness),
)
shield_region = shield_outer["region"] & ~shield_inner["region"]
...
shield_cell = mcdc.Cell(name="Local Shield", region=shield_region, fill=aluminum)
...
mcdc.settings.output_name = "case_B"
...
geo_viewer_3d(simulation=simulation,save_animation_path=str(Path(__file__).with_name("case_B_geometry.gif")))
