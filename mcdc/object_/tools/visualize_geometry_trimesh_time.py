"""Time-dependent trimesh-based geometry visualization for MCDC."""
...
def geo_viewer_3d_time_trimesh(
    simulation,
    ...
    print(f"[trimesh] Precomputing {len(times)} frames...")
    for idx, t in enumerate(times):
        print(f"[trimesh]   frame {idx + 1}/{len(times)} (t={t})")
    ...
    if save_animation_path:
        print(f"[trimesh] Saving animation to {save_animation_path}")
        export_frame_cache_animation(
            pv=pv,
