...
@njit
def _get_crossing_top_cell_IDs(P_arr, simulation, data):
    P = P_arr[0]
    x = P["x"]
    y = P["y"]
    z = P["z"]
    ...
    P["y"] = y - epsilon * P["uy"]
    P["z"] = z - epsilon * P["uz"]
    pre_cell_ID = geometry.get_cell(P_arr, UNIVERSE_ROOT, simulation, data)

    P["x"] = x + epsilon * P["ux"]
    P["y"] = y + epsilon * P["uy"]
    P["z"] = z + epsilon * P["uz"]
    post_cell_ID = geometry.get_cell(P_arr, UNIVERSE_ROOT, simulation, data)
